...
const LEAD = 6; // sub-cat, category, brand, commercial, mont avg, weekly avg
      ...
      {g.show_total && (
        <>
          <tr style={{ background: hex(pal.mbtotal), fontWeight: 700 }}>
            <td /><td /><td className="txt">Total {g.mother_brand}</td><td />
          ...
          <tr style={{ background: hex(pal.acd) }}>
            <td /><td /><td className="txt">Total {g.mother_brand} - ACD</td><td /><td /><td />
...
function Brand({ b, months, category, subcat, pal }) {
  const rows = [];
  ...
  const simpleRow = (label, data, opts = {}, style = {}) => (
    <tr style={style}>
      <td /><td /><td /><td className="label">{label}</td><td /><td />
